...
enum Buffer_IDs { ArrayBuffer, NormalBuffer, TexCoords, NumBuffers };
enum Attrib_IDs { vPosition, vNormal, vTexCoords };
...
GLfloat **readFile(const char *fileName) {
  ...
  if (file) {
    ...
    if (file >> s >> s >> s >> numTriangles) {
      ...
      GLfloat *texcoords = (GLfloat *)malloc(NumVertices * 2 * sizeof(GLfloat));

      GLfloat x, y, z, nx, ny, nz, tu, tv;
      ...
      for (int i = 0; i < numTriangles; i++) {
        ...
        for (int j = 0; j < 3; j++) {
          if (hasTex) {
            file >> s >> x >> y >> z >> nx >> ny >> nz >> s >> tu >> tv;
            texcoords[i * 6 + j * 2] = tu;
            texcoords[i * 6 + j * 2 + 1] = tv;
          } else {
      ...
      GLfloat **attrSet = (GLfloat **)malloc(3 * sizeof(GLfloat *));
      attrSet[0] = vertices;
      attrSet[1] = normals;
...
GLfloat *close2GLTriangleProcessing(GLfloat *vtxArray, GLfloat *normalArray,
                                    GLfloat *texCoordArray, float scale) {
  ...
  for (int i = 0; i < numTriangles; i++) {
    ...
    glm::vec2 texcoords[3] = {
        glm::vec2(texCoordArray[i * 6], texCoordArray[i * 6 + 1]),
        glm::vec2(texCoordArray[i * 6 + 2], texCoordArray[i * 6 + 3]),
        glm::vec2(texCoordArray[i * 6 + 4], texCoordArray[i * 6 + 5]),
    };

    Triangle triangle(vtxs, normals, texcoords);
    ...
    triangle.rasterize(
        colors, zbuffer, width, height, options->getShaderMode(),
        options->getRasterMode(), glm::vec4(options->getAmbient(), 1.0f),
        glm::vec4(camera.getPosition(), 1.0f), glm::vec4(lightPosition, 1.0f),
        M, P * V * M, Viewport, options->getShouldUseTexture(),
        options->getTextureMode(), image, im_width, im_height, image_mipmap);
  }
...
void setupColor(GLuint program, GLfloat *vertices, GLfloat *normals,
  ...
  GLuint usesTextureID = glGetUniformLocation(program, "usesTexture");
  glUniform1i(usesTextureID, options->getShouldUseTexture());
  ...
  if (options->getUsingClose2GL()) {
    GLfloat *colors = close2GLTriangleProcessing(vertices, normals, texcoords,
                                                 adjustmentScale);
    float z = 0.0f;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, im_width, im_height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, image);
    glGenerateMipmap(GL_TEXTURE_2D);
    switch (options->getTextureMode()) {
  ...
  int location;
  location = glGetUniformLocation(program, "ourTexture");
  glUniform1i(location, 0);

  location = glGetUniformLocation(program, "image");
...
void setupDisplay(GLfloat *vertices, GLfloat *normals, GLfloat *texcoords) {
  ...
  setupColor(program, vertices, normals, texcoords);
}

void init(GLfloat *vertices, GLfloat *normals, GLfloat *texcoords) {
  ...
  if (options->getUsingClose2GL()) {
    ...
    glBufferStorage(GL_ARRAY_BUFFER, NumVertices * 2 * sizeof(GLfloat),
                    texcoords, 0);
...
void printTutorial() {
  const char *tutorialText = "=== Commands ===\n"
...
void generateMipmapLevels(int levels) {
  std::cout << "AA" << std::endl;
  image_mipmap = (uint8_t **)malloc(levels * sizeof(uint8_t *));
  image_mipmap[0] = image;
  int in_w, in_h, out_w, out_h, in_idx, y_offset, out_idx;
  glm::vec4 colors[4], out_color;
  for (int i = 1; i < levels; i++) {
    in_h = (im_height / glm::pow(2.0f, i - 1));
    in_w = (im_width / glm::pow(2.0, i - 1));
    out_h = (im_height / glm::pow(2.0f, i));
    out_w = (im_width / glm::pow(2.0, i));

    image_mipmap[i] = (uint8_t *)malloc(out_w * out_h * 4);

    for (int y = 0; y < out_h; y++) {
      for (int x = 0; x < out_w; x++) {

        in_idx = y * 2 * in_w * 4 + x * 2 * 4;
        out_idx = y * out_w * 4 + x * 4;

        colors[0] = glm::vec4(
            image_mipmap[i - 1][in_idx], image_mipmap[i - 1][in_idx + 1],
            image_mipmap[i - 1][in_idx + 2], image_mipmap[i - 1][in_idx + 3]);

        colors[1] = glm::vec4(
            image_mipmap[i - 1][in_idx + 4], image_mipmap[i - 1][in_idx + 5],
            image_mipmap[i - 1][in_idx + 6], image_mipmap[i - 1][in_idx + 7]);
        ...
        out_color = (colors[0] + colors[1] + colors[2] + colors[3]) / 4.0f;
        image_mipmap[i][out_idx] = out_color[0];
        image_mipmap[i][out_idx + 1] = out_color[1];
        image_mipmap[i][out_idx + 2] = out_color[2];
        image_mipmap[i][out_idx + 3] = out_color[3];
      }
    }
  }
}
...
void glEventLoop(char **argv) {
  ...
  GLfloat *texcoords = attrSet[2];

  if (hasTex) {
    int nChannels;

    image = stbi_load(texture_filename, &im_width, &im_height, &nChannels, 4);

    generateMipmapLevels(8);
  ...
  if (!options->getUsingClose2GL())
    init(vertices, normals, texcoords);
  ...
  GLfloat *cptexcoords = (GLfloat *)malloc(NumVertices * 2 * sizeof(GLfloat));
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    if (options->getShouldSwitchRenderMode() || options->getUsingClose2GL()) {
      ...
      if (hasTex)
        memcpy(cptexcoords, texcoords, NumVertices * 2 * sizeof(GLfloat));

      init(cpvertices, cpnormals, texcoords);
      NumVertices = TrueNumVertices;
    }

    glfwGetWindowSize(window, &width, &height);

    setupDisplay(cpvertices, cpnormals, cptexcoords);
...
int main(int argc, char **argv) {
  ...
  OptionWindow opwin(options, hasTex);
